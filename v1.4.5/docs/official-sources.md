# Official Sources

v1.4 콘텐츠 작성 시 확인한 OpenAI 공식 Codex 문서입니다.

- Codex Glossary: https://developers.openai.com/codex/glossary
- Codex Best practices: https://developers.openai.com/codex/learn/best-practices
- Codex app: https://developers.openai.com/codex/app
- Codex CLI: https://developers.openai.com/codex/cli
- Codex IDE extension: https://developers.openai.com/codex/ide
- Agent Skills: https://developers.openai.com/codex/skills
- Agent approvals & security: https://developers.openai.com/codex/agent-approvals-security#sandbox-and-approvals
- config.toml reference: https://developers.openai.com/codex/config-reference#configtoml

Knox, GSCM, Knox Portal, SMTP, DRM 관련 내용은 OpenAI 공식 문서가 아니라 교육용 업무 시나리오와 보안 주의사항으로 작성했습니다.
